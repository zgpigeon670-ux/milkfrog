...
Game View 的英文文字避免依赖额外中文字体。上方面板显示状态、剩余时间、生命、架势、弹反窗口和攻击编号；下方面板显示 AI 决策和连续格挡计数。

动画场景中玩家为青色、敌人为红色，地面矩形用金色/红色/棕色区分前摇/有效期/后摇。原胶囊场景 `CombatDemo.unity` 保留为回归基线，其角色本体按状态变色。选中角色，在 Scene View 开启 Gizmos 可检查攻击范围和防御角度。
...
参数应保持合理非负值，生命和最大架势大于零。距离按角色中心的水平距离计算，近战还必须与前方检测盒接触。处决使用水平距离和正面 120° 检查。
...
- `CombatActorView`、`CombatDebugHud`、`DemoCamera`：表现与调试，不拥有战斗规则。
...
- 闪光复用 8 个池对象；短音效由正弦包络合成。相机跟随双方并做 SphereCast 避障，冲击幅度有上限。
- 世界标签只在状态/弹反窗口改变时重写，HUD 文本最多 10 Hz；F2 隐藏面板。地面预警框与武器拖尾辅助观察攻击窗口。
- 表现中断后停止旧攻击拖尾；处决仅组合挥剑和死亡，不是精密双人对位演出。命中位置当前取双方躯干中点，仍使用原有前方范围检测，而非刀刃骨骼碰撞。

## 构建、截图和性能复现

以下工具是批处理验证入口，应在单独项目副本运行，不关闭或占用正在编辑的原项目：

- `Milkfrog.CombatDemo.Editor.CombatDemoCapture.RunAnimated`：进入 Play Mode，等待着色器完成，记录待机/防御、有效期、被弹反、架势崩溃截图。
...
详见 [第二阶段验证报告](Validation/Animated/Report.md) 与 [第三方许可和固定来源](ThirdParty/SOURCES.md)。
