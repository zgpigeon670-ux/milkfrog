...
当前 Windows 构建位于 [MilkfrogMVP-Phase9](../../Builds/MilkfrogMVP-Phase9/MilkfrogMVP.exe)，也可直接在 Unity 中运行首页场景。本轮验证见 [交互、任务与世界存档报告](Validation/WorldQuest/Report.md)。新增任务测试通过，但完整回归仍有原有战斗测试失败，详见报告；本构建用于功能验收。此前的 [篝火快速传送报告](Validation/FastTravel/Report.md)保留为历史记录。

## MVP：从首页开始

打开 `Scenes/MainMenu.unity` 并 Play，或运行项目 `Builds/MilkfrogMVP-Phase9/MilkfrogMVP.exe`。开始游戏有有效存档时继续，无档时建立新游戏；新游戏按钮会确认覆盖已有存档。
...
平地从南向北依次布置三名巡逻小怪和训练守卫 BOSS。探索时每秒恢复 5 生命、20 架势；普通交战连续 10 秒无有效攻防或离开追击区域后结束。BOSS 在封印门开启后才允许于 8m 内触发，自动锁定并封闭场地，直到一方死亡。
...
关卡有起点和 BOSS 区域外两处篝火。休息时补满生命、清空架势并重置普通小怪；篝火菜单的“快速传送”仅列出已点亮的其他篝火，抵达后该处成为新复活点，也会补满资源并重置普通小怪。传送写盘失败时不会移动角色。死亡后在最近激活的篝火以满血复活，保留经验与加点，普通小怪重生，已击败的 BOSS 不会重生。小怪每次击败给 20 经验，BOSS 首次击败给 200。篝火菜单可加体魄（每点最大生命 +10）、定力（每点最大架势 +10）和攻击（每点进攻生命与架势伤害 +5%）；每项最多 10 点，消耗为 `40 + 20 ×（当前等级 − 1）`。

v3 存档位于 `Application.persistentDataPath/MilkfrogMVP/save.json`，另有备份文件；记录安全位置、朝向、生命、架势、击败敌人 ID、通关标志、篝火 ID 和成长数据。仅脱战且动作 Neutral 时保存位置；战斗奖励只更新最近安全档的成长数据。死亡重试改为篝火复活，不恢复攻击、锁定和仇恨等瞬间状态。新游戏、脱战、篝火休息、加点、BOSS 胜利自动保存，探索中每 15 秒保存，Esc 菜单也提供手动保存。失败会显示提示；损坏文件保留，优先恢复备份。旧 v1 档读取时保留位置与资源、默认起点篝火，已通关档补发一次 200 经验；成功写入当前版本后不再重复补发。
...
首版任务：点亮守门篝火 → 取得守门钥匙 → 开启封印门 → 击败守门人。钥匙位于守门篝火东侧 6m，门位于 BOSS 南侧触发圈外。允许提前拿钥匙；HUD 显示最早未完成的目标。门未开启时从任何方向靠近均不能触发或伤害 BOSS。任务没有额外经验奖励。

靠近目标并使其进入视野，在屏幕下方出现提示时按 E。篝火、钥匙和门共用入口：距离不超过 2.5m、高度差不超过 1.5m、无遮挡，按画面中心距离、玩家距离和稳定 ID 选择唯一目标。只有安全脱战且处于 Neutral 时可执行；暂停、死亡、顿帧期间不缓存 E。Esc →“任务 / 属性”→“任务进度”查看四步记录。

v3 增加已拾取对象、关键物品、永久开门和任务进度。拾取／开门／点亮篝火先保存后生效，失败可重试；死亡、休息、传送不清除任务物品或重新关门。v1／v2 自动迁移，未通关旧档按已点亮篝火继续；已通关旧档视为已经完成试炼，补齐守门篝火、钥匙和门记录，不新增 BOSS 经验。保存失败的通关页提供“重试保存”。

`Settings/GuardianTrial.asset` 配置任务步骤；`GuardianKey`、`GuardianGate` Prefab 是可编辑占位物件。`Tools > Combat Demo > Upgrade MVP Quest` 仅补建缺失内容和引用，保留手工位置。持久化对象 stableId 不可重复、不可随意改名；构建检查会拒绝缺失／重复 ID 和无法解析的任务目标。篝火采用 `IInteractable`，后续交互可实现相同接口；世界事实由 `WorldStateService` 管理，任务由 `QuestProgressService` 计算。

## 原训练场开始方式

1. 等待 Unity 导入脚本并完成编译。
2. 打开 **`Assets/CombatDemo/Scenes/CombatDemo_Animated.unity`**，点击 Play，再点击 Game View 使其获得输入焦点。
3. WASD 相对镜头移动，Tab 开关锁定朝向。短按左键斩击，第一刀结束后 0.18 秒内再按左键接第二刀；长按左键蓄力、松开突刺。Shift＋方向垫步，无方向时后撤；空格在可行动且着地时跳跃。右键按住格挡，命中前 0.15 秒内新按下可弹反。斩击后摇前 0.16 秒、突刺后摇前 0.14 秒可以用格挡或垫步取消。
...
准备／蓄力可以用右键或 Shift 取消；释放后的斩击和突刺不能主动取消，受击与弹反仍可中断。空格跳跃只在可行动且着地时生效，攻击、闪避和受击中不跳。训练场双方与 MVP 小怪没有霸体；MVP BOSS 的普通受击免硬直由其独立配置启用。战斗没有反伤。同一攻击每个 CombatCore 最多命中一次，多个子 Collider 不会重复扣血。双方同一时间发起攻击时，本 Demo 固定先推进玩家，后推进敌人；不实现联网回滚或双向同时命中裁决。
