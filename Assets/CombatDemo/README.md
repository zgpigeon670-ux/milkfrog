...
准备／蓄力可以用右键或空格取消；释放后的斩击和突刺不能主动取消，受击与弹反仍可中断。训练场双方与 MVP 小怪没有霸体；MVP BOSS 的普通受击免硬直由其独立配置启用。战斗没有反伤。同一攻击每个 CombatCore 最多命中一次，多个子 Collider 不会重复扣血。双方同一时间发起攻击时，本 Demo 固定先推进玩家，后推进敌人；不实现联网回滚或双向同时命中裁决。
