...
第三阶段新增 `Animations/Directional` 下的六个后退／侧移走跑片段：在上述 CC0 Mannequin / Sword_Idle 基础上，由本项目 `CombatMotionAuthoring` 通过腿部逆运动学和 Humanoid 肌肉关键帧制作。跳跃使用原包 `Jump_Start`、`Jump_Loop`、`Jump_Land`，四向闪避共用原包 `Roll`，点按与蓄力挥砍都使用 `Sword_Attack`。这些片段都来自同一 CC0 FBX，没有额外下载动画。
